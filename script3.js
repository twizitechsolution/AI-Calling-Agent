...
// ΓöÇΓöÇ Navigation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Stats & Dashboard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function loadDashboard() {
  try {
    ...
    document.getElementById('stat-calls').textContent = stats.total_calls ?? 'ΓÇö';
    document.getElementById('stat-bookings').textContent = stats.total_bookings ?? 'ΓÇö';
    document.getElementById('stat-duration').textContent = stats.avg_duration ? stats.avg_duration + 's' : 'ΓÇö';
    document.getElementById('stat-rate').textContent = stats.booking_rate ? stats.booking_rate + '%' : 'ΓÇö';
    ...
    tbody.innerHTML = logs.slice(0, 10).map(log => `
      <tr>
        ...
        <td>
          ${log.id ? `<a style="color:var(--accent);font-size:12px;text-decoration:none;" href="/api/logs/${log.id}/transcript" download="transcript_${log.id}.txt">Γ¼ç Download</a>` : ''}
        </td>
      </tr>`).join('');
  } catch(e) {
    document.getElementById('dash-table-body').innerHTML = '<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--muted);">Could not load data ΓÇö check Supabase credentials.</td></tr>';
  }
}

function badgeFor(summary) {
  if (!summary) return '<span class="badge badge-gray">Ended</span>';
  if (summary.toLowerCase().includes('confirm')) return '<span class="badge badge-green">Γ£ô Booked</span>';
  if (summary.toLowerCase().includes('cancel')) return '<span class="badge badge-yellow">Γ£ù Cancelled</span>';
  return '<span class="badge badge-gray">Completed</span>';
}

// ΓöÇΓöÇ Call Logs ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  try {
    ...
    tbody.innerHTML = logs.map(log => `
      <tr>
        ...
        <td>${log.duration_seconds || 0}s</td>
        <td>${badgeFor(log.summary)}</td>
        <td style="color:var(--muted);font-size:12px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${log.summary || ''}">${log.summary || 'ΓÇö'}</td>
        <td>
          ${log.id ? `<a class="btn btn-ghost btn-sm" style="text-decoration:none;" href="/api/logs/${log.id}/transcript" download="transcript_${log.id}.txt">Γ¼ç Transcript</a>` : 'ΓÇö'}
          ${log.recording_url ? `<a class="btn btn-ghost btn-sm" style="text-decoration:none;margin-left:4px;" href="${log.recording_url}" target="_blank">≡ƒÄº Recording</a>` : ''}
        </td>
      </tr>`).join('');
  } catch(e) {
...
// ΓöÇΓöÇ Calendar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function openDayModal(dateStr, bookings) {
  ...
  if (!bookings || bookings.length === 0) {
    document.getElementById('modal-bookings-body').innerHTML =
      '<div style="text-align:center;padding:32px;color:var(--muted);font-size:14px;">≡ƒôà No bookings on this day.</div>';
  } else {
    document.getElementById('modal-bookings-body').innerHTML = bookings.map(b => `
      <div class="booking-item">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <div style="font-weight:700;font-size:14px;">≡ƒô₧ ${b.phone_number || 'Unknown'}</div>
          <span class="badge badge-green">Γ£à Booked</span>
        </div>
        <div style="font-size:12px;color:var(--muted);margin-top:6px;">≡ƒòÉ ${new Date(b.created_at).toLocaleTimeString('en-IN', {hour:'2-digit',minute:'2-digit'})}</div>
        ${b.summary ? `<div style="font-size:12px;color:var(--text);margin-top:6px;padding:8px;background:rgba(255,255,255,0.04);border-radius:6px;">≡ƒÆ¼ ${b.summary}</div>` : ''}
      </div>`).join('');
  }
...
// ΓöÇΓöÇ CRM ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  try {
    ...
    tbody.innerHTML = contacts.map(c => `
      <tr style="border-bottom:1px solid var(--border);transition:background 0.12s;" onmouseover="this.style.background='rgba(255,255,255,0.025)'" onmouseout="this.style.background=''">
        ...
        <td style="padding:14px 16px;font-family:monospace;font-size:13px;">${c.phone_number || 'ΓÇö'}</td>
        ...
        <td style="padding:14px 16px;color:var(--muted);font-size:12px;">${c.last_seen ? new Date(c.last_seen).toLocaleString('en-IN') : 'ΓÇö'}</td>
        <td style="padding:14px 16px;">${c.is_booked
          ? '<span class="badge badge-green">Γ£à Booked</span>'
          : '<span class="badge badge-gray">≡ƒô╡ No booking</span>'}</td>
      </tr>`).join('');
  } catch(e) {
...
// ΓöÇΓöÇ Save Config ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Language Presets ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const LANG_PRESETS = {
  hinglish:    { flag:'≡ƒç«≡ƒç│', label:'Hinglish',                sub:'Hindi + English mix',        color:'#6c63ff' },
  hindi:       { flag:'≡ƒç«≡ƒç│', label:'Hindi',                   sub:'Pure Hindi',                  color:'#a855f7' },
  english:     { flag:'≡ƒç¼≡ƒçº', label:'English (India)',          sub:'Indian English',              color:'#3b82f6' },
  tamil:       { flag:'≡ƒç«≡ƒç│', label:'Tamil',                   sub:'α«ñα««α«┐α«┤α»ì',                       color:'#f59e0b' },
  telugu:      { flag:'≡ƒç«≡ƒç│', label:'Telugu',                  sub:'α░ñα▒åα░▓α▒üα░ùα▒ü',                      color:'#10b981' },
  gujarati:    { flag:'≡ƒç«≡ƒç│', label:'Gujarati',                sub:'α¬ùα½üα¬£α¬░α¬╛α¬ñα½Ç',                     color:'#ef4444' },
  bengali:     { flag:'≡ƒç«≡ƒç│', label:'Bengali',                 sub:'αª¼αª╛αªéαª▓αª╛',                       color:'#f97316' },
  marathi:     { flag:'≡ƒç«≡ƒç│', label:'Marathi',                 sub:'αñ«αñ░αñ╛αñáαÑÇ',                       color:'#14b8a6' },
  kannada:     { flag:'≡ƒç«≡ƒç│', label:'Kannada',                 sub:'α▓òα▓¿α│ìα▓¿α▓í',                       color:'#8b5cf6' },
  malayalam:   { flag:'≡ƒç«≡ƒç│', label:'Malayalam',               sub:'α┤«α┤▓α┤»α┤╛α┤│α┤é',                      color:'#ec4899' },
  multilingual:{ flag:'≡ƒîì', label:'Multilingual (Auto)',       sub:"Detects caller's language",   color:'#22c55e' },
};
...
function renderLangGrid() {
  ...
  grid.innerHTML = Object.entries(LANG_PRESETS).map(([id, p]) => `
    ...
    " onmouseover="this.style.borderColor='${p.color}'" onmouseout="this.style.borderColor='${id===currentLangPreset?p.color:'var(--border)}'">
      ...
      ${id===currentLangPreset ? '<div style="font-size:10px;color:#22c55e;margin-top:6px;font-weight:600;">Γ£ô ACTIVE</div>' : ''}
    </div>`).join('');
}
...
async function selectLangPreset(id) {
  ...
  try {
    ...
    toast.textContent = `Γ£à ${p.label} preset activated!`;
    document.body.appendChild(toast);
    setTimeout(()=>toast.remove(), 2500);
  } catch(e) { alert('Failed to save: ' + e); }
}

// ΓöÇΓöÇ Outbound Calls ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ 
async function makeSingleCall() {
  ...
  try {
    ...
    if (res.status === 'ok') {
      el.innerHTML = `Γ£à Call dispatched! Dispatch ID: <code>${res.dispatch_id}</code>`;
      el.style.color = 'var(--green)';
    } else {
      el.textContent = 'Γ¥î ' + res.message;
      el.style.color = 'var(--red)';
    }
  } catch(e) {
    el.textContent = 'Γ¥î Error: ' + e;
    el.style.color = 'var(--red)';
  }
}
...
async function makeBulkCall() {
  ...
  el.textContent = 'ΓÅ│ Dispatching all numbers...';
  try {
    ...
    document.getElementById('call-results-body').innerHTML = results.map(r => `
      <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);">
        ...
        <span class="badge ${r.status==='ok'?'badge-green':'badge-gray'}">${r.status==='ok'?'Γ£à Sent':'Γ¥î '+r.message}</span>
      </div>`).join('');
    el.textContent = `Γ£à ${results.filter(r=>r.status==='ok').length}/${results.length} calls dispatched`;
    el.style.color = 'var(--green)';
  } catch(e) {
    el.textContent = 'Γ¥î Error: ' + e;
    el.style.color = 'var(--red)';
...
// ΓöÇΓöÇ Demo Link ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
let demoUrl = '';
function initDemo() {
...
async function generateDemo() {
  ...
  statusEl.textContent = 'ΓÅ│ Generating session...';
  try {
    ...
    statusEl.textContent = 'Session ready ΓÇö share the link or use the preview below';
  } catch(e) {
    statusEl.textContent = 'Γ¥î ' + e;
  }
}
function copyDemoLink() {
  navigator.clipboard.writeText(demoUrl);
  document.getElementById('copy-demo-btn').textContent = 'Γ£à Copied!';
  setTimeout(()=>document.getElementById('copy-demo-btn').textContent='≡ƒôï Copy Link', 2000);
}

// ΓöÇΓöÇ Boot ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
loadDashboard();
